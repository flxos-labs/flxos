        ...
        switch ((s16_t) *optptr) {
            ...
            case HOST_NAME: { // 12
                ...
                while (pnode != NULL) {
                    ...
                    // We need the MAC address to associate hostname.
                    // But parse_options is called after parse_msg has already found/created the pool entry for the current MAC.
                    // The pool entry's IP is already set to client_address.
                    if (pdhcps_pool->ip.addr == client.addr) {
                        memcpy(pdhcps_pool->hostname, optptr + 2, l);
                        pdhcps_pool->hostname[l] = '\0';
                        break;
                    }
                    pnode = pnode->pnext;
                }
...
int dhcps_get_active_leases(dhcp_lease_info_t *leases, int max_leases)
{
    ...
    for (pback_node = g_dhcps_instance->plist;
        ...
        if (pdhcps_pool != NULL) {
            ...
            strncpy(leases[count].hostname, pdhcps_pool->hostname, 31);
            leases[count].hostname[31] = '\0';
            count++;
        }
    }
...
bool __wrap_dhcp_search_hostname_on_mac(dhcps_t *dhcps, u8_t *mac, char *hostname)
{
    struct dhcps_pool *pdhcps_pool = NULL;
    list_node *pback_node = NULL;
    bool ret = false;

    if (dhcps == NULL || hostname == NULL) {
        return false;
    }
    ...
    for (pback_node = dhcps->plist; pback_node != NULL; pback_node = pback_node->pnext) {
        ...
        if (memcmp(pdhcps_pool->mac, mac, 6) == 0) {
            strncpy(hostname, pdhcps_pool->hostname, 31);
            hostname[31] = '\0';
            ret = true;
            break;
        }
