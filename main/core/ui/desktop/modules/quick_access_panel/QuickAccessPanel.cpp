...
QuickAccessPanel::QuickAccessPanel(lv_obj_t* parent, lv_obj_t* dock)
    ...
    if (!m_parent || !m_dock) {
        LV_LOG_ERROR("QuickAccessPanel: Parent or Dock is NULL");
        return;
    }
    create();
}
...
void QuickAccessPanel::buildHeader() {
    ...
    lv_obj_add_event_cb(settings_btn, [](lv_event_t* e) {
        ...
        System::Apps::AppManager::getInstance().startApp(
            System::Apps::Intent::forApp("com.flxos.settings")
        ); 
        ...
        if (shot_cont) {
            ...
            if (shot_btn) {
                ...
                lv_obj_add_event_cb(shot_btn, [](lv_event_t* e) {
                    ...
                    System::Services::ScreenshotService::getInstance().scheduleCapture(delaySec); 
                }, LV_EVENT_CLICKED, nullptr);
            }
