...
void WindowManager::openApp(const std::string& packageName) {
	...
	flx::ui::FocusManager::getInstance().registerWindow(win);
	flx::ui::FocusManager::getInstance().activateWindow(win);
...
lv_obj_t* WindowManager::createAndConfigureAppButton(lv_obj_t* win, flx::apps::App* app) {
	...
	lv_subject_add_observer_obj(
		flx::ui::theming::UiThemeManager::getInstance().getTransparencyEnabledSubject(),
