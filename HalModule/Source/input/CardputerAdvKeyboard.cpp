...
lv_indev_t* CardputerAdvKeyboard::getLvglIndev() const {
	return m_indev;
}

int CardputerAdvKeyboard::subscribeKeyEvents(KeyEventCallback cb) {
	std::lock_guard<std::mutex> lock(m_mutex);
...
void CardputerAdvKeyboard::unsubscribeKeyEvents(int id) {
	std::lock_guard<std::mutex> lock(m_mutex);
...
void CardputerAdvKeyboard::updateKeysState() {
...
void CardputerAdvKeyboard::notifyObservers(const KeyEvent& event) {
...
void CardputerAdvKeyboard::indevReadCb(lv_indev_t* indev, lv_indev_data_t* data) {
	auto* device = static_cast<CardputerAdvKeyboard*>(lv_indev_get_user_data(indev));
	if (device) {
		device->readInput(data);
	}
}

void CardputerAdvKeyboard::readInput(lv_indev_data_t* data) {
	updateKeyList();
